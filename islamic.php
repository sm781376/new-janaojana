...
<!--Start Side Bar-->
		<div id="sidebar">
			...
			if(isset($_GET['catid'])){
					$catid=$_GET['catid'];
					$query= "SELECT * FROM index_tbl WHERE category='$catid' order by id desc limit 3";
					$post=$db->select($query);
					if($post){
					while($result=mysqli_fetch_array($post)){
			?>
			<div id="onesidebar"><a  href="post.php?postid=<?php echo $result['id'];?>">
				<img class='photo' src="admin/<?php echo $result['image'];?>"/></a>
...
<div id="container">
		<?php
			if(isset($_GET['catid'])){
					$catid=$_GET['catid'];
					$query= "SELECT * FROM index_tbl WHERE category='$catid' order by id desc ";
					$post=$db->select($query);
					if($post){
					while($result=mysqli_fetch_array($post)){
			?>
